...
class Faster_test_model(model):
    ...
    def create_model(self):
        ...
        (self.get_dt('rpn_cls_prob_reshape', 'rpn_bbox_pred', 'im_info')
         ...
         .proposal_layer(_feat_stride, anchor_scales, 'TEST', name = 'roi-data'))

        # ========= RCNN ============
        (self.get_dt('conv5_3', 'roi-data')
            .roi_pool(7, 7, 1.0 / 16, name='pool_5').get_dt('pool_5')
            .fc(4096, name='fc6').get_dt('fc6')
            .fc(4096, name='fc7').get_dt('fc7')
            .fc(n_classes, relu=False, name='cls_score').get_dt('cls_score')
            .softmax(name='cls_prob'))

        (self.get_dt('fc7').fc(n_classes * 4, relu=False, name='bbox_pred'))
    def test_image(self,sess, image, im_info):
        self.cls_score = self.get_output("cls_score")
        self.cls_prob  = self.get_output('cls_prob')
        self.bbox_pred = self.get_output('bbox_pred')
        self.rpn_rois = self.get_output('roi-data')
        self.temp = self.get_output('rpn_cls_prob')

        feed_dict = {self.data: image, self.im_info:im_info, self.keep_prob: 1.0}

        cls_score, cls_prob, bbox_pred, rois,temp = sess.run([self.cls_score,
                                                         self.cls_prob,
                                                         self.bbox_pred,
                                                         self.rpn_rois,
                                                         self.temp],

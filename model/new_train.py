...
class Do_Train(object):
    ...
    def __init__(self, sess, saver, model, my_roidb, output_dir, pretrained_model=None):
        self.model = model
        #self.imdb = imdb
        self.my_roidb = my_roidb
       ...
       #True
        if cfg.TRAIN.BBOX_REG:
            ...
            self.bbox_means, self.bbox_stds = rdl_roidb.add_bbox_regression_targets(my_roidb.data_inof.roidb)
        print('done')
        # For checkpoint
        self.saver = saver
    ...
    def train_faster_model(self, sess, max_iters):
        ...
        if self.pretrained_model is not None:
            ...
            variables_to_restore = self.model.get_variables_to_restore(variables, var_keep_dic)
        ...
        for iter in range(start,max_iters):
            ...
            rpn_loss_cls_value, rpn_loss_box_value,loss_cls_value, loss_box_value, _,label,cls_score = sess.run([self.model.rpn_cross_entropy, self.model.rpn_loss_box,
                                                                                                 self.model.cross_entropy, self.model.loss_box,
                                                                                                 self.model.train_op,
                                                                                                 self.model.label,
                                                                                                 self.model.cls_score],
                                                                                                feed_dict=feed_dict,
                                                                                                options=run_options,
...
def train_faster_model(network, my_roidb, output_dir, pretrained_model=None, max_iters=400000):
    """Train a Fast R-CNN network."""
    #筛选roidb（至少有一个前景或者背景的图片）
    my_roidb.data_inof.roidb = filter_roidb(my_roidb.data_inof.roidb)
    #对参数进行保存，100次迭代更新一次
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        Do_ = Do_Train(sess, saver, network,my_roidb, output_dir, pretrained_model=pretrained_model)
        print('Solving...')
        Do_.train_faster_model(sess, max_iters)
        print('done solving')

#network为VGGnet_train一个对象，imdb为pascal_voc对象，roidb为一个列表
def train_mask_model(network, my_roidb, output_dir, pretrained_model=None, max_iters=400000):
    """Train a Fast R-CNN network."""
    #筛选roidb（至少有一个前景或者背景的图片）
    my_roidb.data_inof.roidb = filter_roidb(my_roidb.data_inof.roidb)
    #对参数进行保存，100次迭代更新一次

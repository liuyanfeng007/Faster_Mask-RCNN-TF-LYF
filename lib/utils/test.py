...
def test_image(model, sess, image, im_info):
    #feed_dict = {net._image: image,
    #             net._im_info: im_info}
    # 给定placehold信息
    feed_dict = {model.data: image, model.im_info:im_info, model.keep_prob: 1.0}
    ...
    #                                                feed_dict=feed_dict)
    cls_score, cls_prob, bbox_pred, rois = sess.run([model.get_output["cls_score"],
                                                     model.get_output['cls_prob'],
                                                     model.get_output['bbox_pred'],
                                                     model.get_output['rpn_rois']],
                                                    feed_dict=feed_dict)
...
def im_detect(sess, net, im):
    ...
    im_blob = blobs['data']
    # seems to havdemo.pye height, width, and image scales

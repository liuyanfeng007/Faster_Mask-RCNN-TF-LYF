...
import numpy as np
import cv2
import lib.utils.cython_bbox as cython_bbox
#import lib.utils.config.cfg as cfg
from lib.utils.config import cfg
#from libs.logs.log import LOG
from lib.utils.bbox_transform import bbox_transform, bbox_transform_inv, clip_boxes
...
def encode(gt_masks, gt_boxes, rois, num_classes, mask_height, mask_width):
  ...
  if gt_boxes.size > 0: 
      ...
      overlaps = cython_bbox.bbox_overlaps(
          np.ascontiguousarray(rois[:, 1:5], dtype=np.float), ####liuyf
      ...
      if keep_inds.size > 0 and num_masks < keep_inds.size:
        ...
        print('Masks: %d of %d rois are considered positive mask. Number of masks %d'%(num_masks, rois.shape[0], gt_masks.shape[0]))
      ...
      for i in keep_inds:
        ...
        #cropped = gt_masks[gt_assignment[i], int(roi[0]):int(roi[2]) + 1, int(roi[1]):int(roi[3]) + 1]
        #print(cropped.shape[0])
        #print(cropped[0,:])
        cropped = cv2.resize(cropped, (mask_height,mask_width ), interpolation=cv2.INTER_NEAREST)

...
def get_minibatch(roidb, num_classes):
    ...
    masks = _get_mask_blob(roidb,random_scale_inds,im_blob.shape[1], im_blob.shape[2])
    #画像
    blobs = {'data': im_blob}
...
def _get_mask_blob(roidb,scale_inds,widh,hight):
    num_images = len(roidb)
    for iii in range(num_images):
        from PIL import Image
        inof = cv2.imread(roidb[iii]['image'].replace("JPEGImages","SegmentationClass").replace(".jpg",".png"))
        im = Image.open(roidb[iii]['image'].replace("JPEGImages","SegmentationClass").replace(".jpg",".png"))
        target_size = cfg.TRAIN.SCALES[scale_inds[iii]]
        #_,im_scale = prep_im_for_blob_mask(im, target_size, cfg.TRAIN.MAX_SIZE)
        nBoxs = roidb[0]['boxes'].shape[0]
        ...
        for ii in range(nBoxs):
            ...
            x1,y1,x2,y2 = roidb[iii]['boxes'][ii,0:4]
            index = roidb[iii]['gt_classes'][ii]
